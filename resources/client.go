...
func (c *Client) Create(parent context.Context, meta Meta, resource any, opts ...CreateOpt) error {
	//ctx, span := tracing.Start(parent, "resources.Create "+meta.Type.String(), trace.WithSpanKind(trace.SpanKindClient))
	//defer span.End()

	bytes, err := json.Marshal(resource)
	if err != nil {
		//span.SetStatus(codes.Error, "failed to encode resource")
		//span.RecordError(err)
		return err
	}

	return c.CreateBytes(parent, meta, bytes, opts...)
}
